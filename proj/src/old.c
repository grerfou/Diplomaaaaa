...
{
    FILE *f = fopen("assets/position/positions_old.cfg", "r");
    if (!f) return;
...
{
    FILE *f = fopen("assets/position/positions_old.cfg", "w");
    if (!f) return;
    fprintf(f, "%f %f\n", shaderTopLeft.x, shaderTopLeft.y);
    fprintf(f, "%f %f\n", shaderTopRight.x, shaderTopRight.y);
    fprintf(f, "%f %f\n", shaderBottomLeft.x, shaderBottomLeft.y);
...
bool Old_Init(const char *filename)
{
    if (fullText) {
        free(fullText);
        fullText = NULL;
    ...
    retroFont = LoadFont("assets/typo/Minitel.ttf");
    ...
    shader = LoadShader(NULL, "assets/shaders/old.fs");
...
void Old_Draw()
{
    ...
    while (i < currentChar)
    {
        ...
        if (pos.y + charSize.y > 0 && pos.y < screenHeight) {
            DrawTextEx(retroFont, buffer, pos, fontSize, spacing, GREEN);
        }
    ...
    if (!isDone) {
        ...
        if (((int)(blinkTime * 2)) % 2 == 0) { // clignote chaque 0.5s
            Vector2 cursorPos = pos;
            char cursorChar[2] = { '_', '\0' }; // █
            DrawTextEx(retroFont, cursorChar, cursorPos, fontSize, spacing, GREEN);
        }
    }
}
